...
class Game_2048():
    ...
    def step(self, action):
        ...
        max_value = max(self.game_state.flatten())
        ...
        reward=self.reward_for_action
        dead = not self.running
        # print(f"reward object {self.reward_for_action}")
        print(f"Maximum value in this state: {max_value}")
    ...
    while game.running and GAME_MODE=="live":
        for e in pygame.event.get():
            if e.type == pygame.QUIT:
                ...
                try:
                    state,reward,dead,_=game.step(move_list.index(e.key))
                    print(f"state: {state}\n reward: {reward} \t dead? {dead}")
                except:
